%% Angular compensation for phantom measurements
% This script loads reconstructed PA images at multiple wavelengths,
% computes SNR at user-defined points, optimizes a compensation vector
% and performs linear unmixing.
clear all; close all;
select_wv = [3:2:21];
...
%% Optimize compensation vector to match normalized reference spectra
normalize = @(x) x / norm(x);  % L2 norm
% normalize = @(x) (x - min(x)) / norm(x - min(x));


% Objective: fit spectra shape with smoothness penalty
objective = @(comp) ...
...
% Initial guess for compensation
init_comp = ones(size(snr_Ni));

% Bounds to avoid division by zero or overly large values
