...
% %     filename = sprintf('Simulation_size5/fluence_1e9_top_sTO2_%.1f_sBO2_0.6.mat',sTO2); 
   load(filename)
    blood_mu_a = (spectrum_HbO2 .* sBO2 + spectrum_Hb .* (1 - sBO2)) ...
            .* (150/64500) * log(10);
%     SO2_cc_diff(i ,1 ) = linearUnmixing(fluence_spectrum1(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
    SO2_cc_diff(i ,2 ) = linearUnmixing(fluence_spectrum2(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
     SO2_cc_diff(i ,3 ) = linearUnmixing(fluence_spectrum3(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
     SO2_cc_diff(i ,4 ) = linearUnmixing(fluence_spectrum4(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
    SO2_cc_diff(i ,5 ) = linearUnmixing(fluence_spectrum5(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
    SO2_cc_diff(i ,6 ) = linearUnmixing(fluence_spectrum6(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
    SO2_cc_diff(i ,7 ) = linearUnmixing(fluence_spectrum7(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
    SO2_cc_diff(i ,8 ) = linearUnmixing(fluence_spectrum8(sele_wavlen).*blood_mu_a(sele_wavlen),spectrum_HbO2 ,spectrum_Hb);
...
% set(gcf, 'Color', 'w');
% hold off;
save(sprintf('/home/zz111/Spectral_unmixing/Convex-Cone-Approach-main/sO2_predictionDT_deepmore_nonuni_linearunmixing_num%d.mat',save_num),"SO2_cc_diff");
