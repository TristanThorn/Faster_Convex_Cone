...
% filename = sprintf('nirfast_result/nirfast_large_spe_wavelength%d_num%d',700,1);
% load(filename);

% mesh = load_mesh('digital_vessel_shrink_spe_mimicCC_0.2_flip_nirfast_mesh');
% mesh = load_mesh('digital_vessel_com_mimicCC_0.2_flip_nirfast_mesh');
mesh = load_mesh('digital_vessel_com_mimicCC_0.2_flip_nirfast_mesh');
% find(mesh.region==5);
...
selected_indices = find(mesh.nodes(:,1) >=4 & mesh.nodes(:,1) <= 6 & mesh.nodes(:,3) >= 3 & mesh.nodes(:,3) <=12);
num_points = 200;
...
for num =1:200
    ...
    for i = 1:21
        ...
        filename = sprintf('nirfast_result/nirfast_large_comp_wavelength%d_num%d',wavelength,num); 
        load(filename);
...
save('My_ColorBase_large_shrink_comp_200_200_6dt', "My_ColorBase_s");