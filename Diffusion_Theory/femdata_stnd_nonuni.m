function [data,mesh]=femdata_stnd_nonuni(mesh,frequency,wavelength,save_num,save_fluence,sBO2)
...
if nargin == 6
    if save_fluence
        %save_name = sprintf('nirfast_result/nirfast_large_deep_wavelength%d_num%d.mat',wavelength,save_num);
        save_name = sprintf('nirfast_result/nirfas_deep_nonuni_sBO2%.1f_wavelength%d_num%d.mat',sBO2,wavelength,save_num);
        phi_amplitude = abs(data.phi);
        save(save_name,'phi_amplitude');
    end
    clear qvec;
...
if nargin == 5
    if save_fluence
        %save_name = sprintf('nirfast_result/nirfast_large_deep_wavelength%d_num%d.mat',wavelength,save_num);
        save_name = sprintf('nirfast_result/nirfas_deepmore_nonuni_VesselDiff_wavelength%d_num%d.mat',wavelength,save_num);
        phi_amplitude = abs(data.phi);
        save(save_name,'phi_amplitude');
    end
    clear qvec;
